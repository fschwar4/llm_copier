...
function parseInline(text) {
  ...
  parts.forEach(part => {
    ...
    if (part.startsWith('**') && part.endsWith('**')) {
      inlineContent.push({ text: part.slice(2, -2), bold: true });
    } 
...
function parseMarkdownToPdfContent(md) {
  ...
  lines.forEach((line) => {
    ...
    if (ulMatch || olMatch) {
      ...
      inList = true;
      listType = type;
      currentList.push({ text: parseInline(text), margin: [0, 2, 0, 2] });
      return;
    }
    ...
    if (headerMatch) {
      ...
      const headerObj = { 
        text: text, 
        style: styleName,
        // Only add H1 (level === 1) to the Table of Contents
        tocItem: level === 1 
      };
...
export function markdownToPdf(markdown, filename = 'chat-export.pdf') {
  ...
  const docDefinition = {
    ...
    content: [
      // --- TITLE PAGE ---
      { text: title, style: 'reportTitle', margin: [0, 60, 0, 10], alignment: 'center' },
      { text: author, style: 'reportAuthor', alignment: 'center', margin: [0, 0, 0, 5] },
      { text: date, style: 'reportDate', alignment: 'center', margin: [0, 0, 0, 40] },
